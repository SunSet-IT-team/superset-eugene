...
export const getFactIds = async (
  ...
  try {
    if (!id || !type || !column) {
      throw new Error('id, type or column is incorrect');
    }
    const data = await getFactsWithIds(id, type, column);
    const selectedData = data.filter(row =>
      selectedFacts.includes(row[column]),
    );
    ...
    console.error('Error in getFactIds:', error);
    return [];
  }
};